...
- 支援內建更新檢測。
- 提供 macOS 14+ 桌面小工具：額度總覽、Token 趨勢、關鍵指標和 Codex 重設動態。
...
桌面小工具要求 macOS 14+。從包含此修復的版本起，正式 Release 和本機開發建置都會包含 Widget 延伸功能。每個小工具可在系統的「編輯小工具」中獨立選擇 Codex、Grok 或兩者；「Codex 重設動態」僅支援 Codex。升級後，macOS 會從應用程式套件的 `Contents/PlugIns` 註冊正式版小工具。
