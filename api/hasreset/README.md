...
```mermaid
flowchart LR
    schedule["GitHub Actions<br/>UTC 每小时 :17"] --> discover["Grok discovery<br/>x_search + web_search"]
...
- cron：`17 * * * *`（UTC 每小时第 17 分钟；GitHub 实际触发时间可能漂移）
...
- 不要直接提高到 5/15 分钟轮询，也不要在未重新评估条款的情况下商业化或改造成
  通用 API。
- GitHub schedule 可能延迟、丢失运行，或因公共仓库长期无活动被停用；可选的
  `repository_dispatch` 只能降低空窗概率，不能保证准时。客户端的 30 小时
  stale 判断和手动触发仍是必要的故障边界。
