...
- 공식 주간 사용률과 일별 Credits로 이번 주 한도를 추정하고, 이전 추정과 비교해 삭감 여부를 확인합니다. 설정에서 켜고 끌 수 있으며 기본값은 켜짐입니다.
...
- 내장 업데이트 확인을 지원합니다.
- macOS 14+ 데스크톱 위젯으로 할당량 개요, 토큰 추세, 핵심 지표, Codex 초기화 업데이트를 제공합니다.

## 스크린샷

<p align="center">
  <img src="docs/images/1.webp" alt="CodexRunway 할당량 개요" width="260">
  <img src="docs/images/2.webp" alt="CodexRunway 리셋 크레딧 상세" width="260">
  <img src="docs/images/3.webp" alt="CodexRunway API 환산 비용" width="260">
  <img src="docs/images/4.webp" alt="CodexRunway 설정 페이지" width="260">
  <img src="docs/images/5.webp" alt="CodexRunway 다중 계정" width="260">
  <img src="docs/images/6.webp" alt="CodexRunway Grok 할당량" width="260">
</p>
...
CodexRunway는 앱 내 업데이트도 지원합니다. Homebrew가 업그레이드를 확인하고 설치하게 하려면:
...
macOS 14+에서는 이 명령이 별도의 `CodexRunway Dev` 앱과 Widget 확장을 빌드·등록한 뒤 실행합니다. 개발 앱은 `.build/codex-runway-widget-dev/CodexRunway-dev.app`에 있으며 시스템 위젯 갤러리에서 바로 추가할 수 있습니다. 다른 CodexRunway 인스턴스는 먼저 종료하세요. 위젯 없이 패키징되지 않은 명령줄 프로세스만 쓰려면 `CODEX_RUNWAY_DISABLE_DEV_APP=1`을 설정하세요.
...
데스크톱 위젯은 macOS 14 이상이 필요합니다. 이 수정이 포함된 버전부터 공개 릴리스와 로컬 개발 빌드 모두 Widget 확장을 포함합니다. 각 위젯은 위젯 편집에서 Codex, Grok 또는 둘 다를 독립적으로 고를 수 있습니다. Codex 초기화 업데이트는 Codex 전용입니다. 업그레이드 후 macOS는 앱의 `Contents/PlugIns`에서 프로덕션 위젯을 등록합니다.
...
**제어판 → 일반 → 네트워크**에서 **시스템 설정 사용**, **HTTP 프록시**, **SOCKS5 프록시**를 선택합니다. 사용자 지정 프록시는 사용자 이름과 비밀번호 인증을 지원합니다. Codex 요청, 앱에서 시작한 Grok 요청과 CLI 작업, 공개 상태·가격 정보, 업데이트 확인·릴리스 노트·설치 파일 다운로드에 적용됩니다. 브라우저 로그인 페이지와 이미 실행 중인 외부 CLI는 자체 네트워크 설정을 유지하며 시스템 프록시는 변경하지 않습니다.

**프록시 설정 저장**을 누르면 적용되며 진행 중인 업데이트는 시작할 때의 설정을 유지합니다. **연결 테스트**는 현재 초안으로 공개 업데이트 피드만 가져오며 계정 인증 정보를 전송하거나 초안을 저장하지 않습니다. 프록시 인증 정보는 일반 설정과 별도로 macOS 키체인에 저장합니다. 설정 손상, 인증 정보 접근 실패 또는 사용자 지정 프록시 실패 시 직접 연결로 전환하지 않습니다. 설정을 수정하거나 직접 **시스템 설정 사용**으로 변경하세요.
