...
The workflow has two trigger slots per hour at minutes 13 and 43 UTC (primary + backup, off the top of the hour to reduce load-queue drops). Automated runs are throttled to at most one successful execution per UTC hour; if the first slot already succeeded, the backup exits early. It does not retry failed upstream calls. It updates the orphan `gh-pages` branch and deploys Pages only when the status changes or a daily heartbeat is due. GitHub schedules are still not real-time and may be delayed or dropped; use `workflow_dispatch` for a manual catch-up, or an external cron that posts `repository_dispatch` with `event_type=update-hasreset`.
